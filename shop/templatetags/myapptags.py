...
def shopcartcount(userid):
    count = ShopCart.objects.filter(user=userid).count()
    return count
...
#                 parent_id__isnull=True).order_by("id")
...
#                 menu += '\t<a class ="dropdown-toggle" data-toggle="dropdown" aria-expanded="true">' + \
...
#             else:
#                 menu += '\t\t\t\t<li><a href="' + \
#                     reverse('category_products', args=(rs.id, rs.slug)
#                             ) + '">' + rs.title + '</a></li>\n'
#     return menu
